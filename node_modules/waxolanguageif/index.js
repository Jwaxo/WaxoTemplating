...
module.exports = function(parser, contents) { 
	...
	return function(string, content) {
		...
		var elseTypes = parser.searchSpecific(content, ["elseif","else"]);
		var ifTests = Array();
		...
		for (var tests in ifTests[1]) {
			console.log("Tests are :" + ifTests[1][tests]);
		}
		...
		for (var i = 0;i<ifTests.length;i++) {
			...
			if (ifTests[i].type != 'else') {
				var ifFunction = ifTests[i].ifFunction;
